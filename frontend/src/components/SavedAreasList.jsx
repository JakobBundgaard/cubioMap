...
function SavedAreasList({
    ...
    const [openProjectId, setOpenProjectId] = useState(null); // State til at holde styr på åbne projekter

    const toggleProjectDetails = (projectId) => {
        setOpenProjectId((prevId) => (prevId === projectId ? null : projectId)); // Åben/luk projektet
    };
    ...
    return (
        <div className="mt-6">
            ...
            <ul className="mt-4 space-y-4">
                {savedAreas.length > 0 ? (
                    savedAreas.map((area) => (
                        <li key={area.id} className="bg-white rounded-lg p-4 shadow-sm">
                            {/* Område Info */}
                            <div>
                                        ...
                                        {area.projects.map((project) => (
                                            <li
                                                ...
                                                className={`bg-gray-100 p-3 rounded-md ${
                                                    openProjectId === project.id ? "shadow-lg" : ""
                                                }`}
                                            >
                                                {/* Titel og Status */}
                                                <div
                                                ...
                                                {/* Detaljer (kun synlige, hvis åbent) */}
                                                {openProjectId === project.id && (
                                                    <div className="mt-3">
                                                        <p className="text-sm text-gray-700">
                                                            {project.description}
                                                        ...
                                                        <p className="text-sm text-gray-700">
                                                            Startdato: {project.date_initiated}
                                                        </p>
                                                        <p className="text-sm text-gray-700">
                                                            Planlagt Varighed:{" "}
                                                            {project.expected_duration} dage
                                                        </p>
                                                        {project.image_url && (
                                                            <img
                                                        ...
                                                        <div className="flex items-center space-x-2 mt-2">
                                                            <button
                                                                ...
                                                                onClick={() =>
                                                                    startEditingAreaProject(project)
                                                                }
                                                            >
                                                                Rediger
                                                            </button>
